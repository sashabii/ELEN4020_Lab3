...
\begin{document}
	...
	The first algorithm uses the MrJobs MapReduce framework. The mapping function, \textit{mapFn} shown in algorithm 1, takes the input matrix \textit{.txt} files and generates the key, value pairs in terms of columns for matrix A, and the key, value pairs in terms of rows for matrix B. From these generated pairs, the reduce function, \textit{reduceFn} show in algorithm 2, appends the key, value pairs into an array, then multiplies the matricies. This multiplied value  is then added using the \textit{addition} function to get the final result.
